...
#FGSM
def fgm(model, x, eps=0.01, epochs=1, sign=True, clip_min=0, clip_max=1.0):
    xadv = tf.identity(x)
    ybar = model(xadv)
    ...
    loss_fn =  tf.nn.softmax_cross_entropy_with_logits_v2 #tf.nn.sparse_softmax_cross_entropy_with_logits#
    noise_fn = tf.sign
    eps = tf.abs(eps)
    def cond(xadv, i):
...
# read images from dataset
    ...
    X_test0 = mnist.test.images
    X_test = mnist.test.images
    y_test = mnist.test.labels
    X_train =((X_train*10).astype(int)).astype(float)
    X_test  =((X_test*10).astype(int)).astype(float)
...
# STEP 4
    training(sess, ambiente, X_train, y_train, X_validate, y_validate, shuffle=False, batch=128, epochs=10)
    evaluate(sess, ambiente, X_test, y_test)

    for i in range(6):
        eps = (i+1)*0.01
        X_adv = perform_fgsm(sess, ambiente, X_test, eps=eps, epochs=12)
        evaluate(sess, ambiente, X_adv, y_test)
        Data_adv = [X_test, X_adv, y_test]
        with open('Data_adv'+str(i)+'.pkl', 'wb') as f:
